...
namespace glfw
{
 ...
 double getLengthOfVector(Eigen::Vector3d vec) {
 ...
 IGL_INLINE bool Viewer::isIntersectBox(Eigen::AlignedBox3d& box0, Eigen::AlignedBox3d& box1,
	 ...
	 //Tamir
	  //C = data_list[0].getRotation().inverse().cast<double>() * data_list[1].getRotation().cast<double>();
  ...
  IGL_INLINE void Viewer::isIntersection(igl::AABB<Eigen::MatrixXd, 3>* tree0, igl::AABB<Eigen::MatrixXd, 3>* tree1,
	  Eigen::Matrix4d& model0, Eigen::Matrix4d& model1,
	  Eigen::Matrix3d& Rot0, Eigen::Matrix3d& Rot1) {
	  //Eigen::Matrix4d model0 = data_list[0].MakeTrans().cast<double>();
	  //Eigen::Matrix4d model1 = data_list[1].MakeTrans().cast<double>();
	  ...
	  else if (tree0->is_leaf() && tree1->is_leaf() && !move_models) {
	  ...
	  // if the first boxes intersect, go deeper:
	  //if (!move_models) {		  
		 // isIntersection(tree0->m_left, tree1->m_left, model0, model1, Rot0, Rot1);
